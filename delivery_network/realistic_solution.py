...
def cost_traject_realistic(nb_road, nb_truck):
    ...
    edge_road = f"route_edge.{nb_road}.out"
    ...
    with open(data_path + route_name, 'r') as f:
        ...
        for line in content:
            ...
            utility_edge[key_travel] = float(line.split()[2])
    ...
    with open(data_path + route_name, 'r') as f:
        ...
        for line in content:
            ...
            utility_power[key_travel] = (max(utility_power[key_travel]*((0.999)**lenght_path) - dist*0.01, 0), cost_travel, i + 1, max(utility_edge[key_travel]*((0.999)**lenght_path) - dist*0.01, 0)/cost_travel)
    with open(data_path + dist_road, 'r') as f:
        
        """we  read the file with a MST where the weight is the distance of the edges. The minimun power is found and so the price of the truck. The realistic solition is 
    ...
    for k in range(1, nb_travel + 1):
        ...
        listes = [utility_power[k], utility_dist[k], utility_ratio[k], utility_edge[k]]
        utility[k] = max(listes, key=lambda x: x[2])
...
print(knapsack_greedy_realistic(1,1,B))