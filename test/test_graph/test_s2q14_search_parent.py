...
class Test_search_parent(unittest.TestCase):
    ...
    def test_network00(self):
        ...
        g_mst = g.kruskal()
        self.assertEqual(Graph.search_parent(g_mst)[0], {1: 0, 2: 1, 3: 2, 4: 3, 5: 2, 6: 1, 7: 3, 8: 1, 9: 2, 10: 4})
        self.assertEqual(Graph.search_parent(g_mst)[1], {8: (1, 0, 1), 9: (8, 14, 1), 2: (1, 11, 1), 5: (2, 4, 1), 7: (5, 14, 1), 3: (2, 10, 1), 4: (3, 4, 1), 10: (4, 4, 1), 6: (1, 12, 1)})

    def test_network01(self):
        g = graph_from_file(data_path +"input/network.05.in")
        g_mst = g.kruskal()
        self.assertEqual(Graph.search_parent(g_mst)[0], {1: 0, 2: 1, 3: 1, 4: 1})
        self.assertEqual(Graph.search_parent(g_mst)[1], {3: (1, 2, 1), 4: (1, 4, 1), 2: (1, 6, 1)})
